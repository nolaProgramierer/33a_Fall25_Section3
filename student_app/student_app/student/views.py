...
def inactive_students(request):
    students = Student.objects.filter(active=False).order_by("lname")
    return render(request, "student/inactive_students.html", {"students": students})
...
def advisor_detail(request, id):
    ...
    addvisee_count = advisor.advisee_count()
    return render(request, 
                  "student/advisor_detail.html", 
                  {"advisor": advisor, "advisee_count": addvisee_count})
...
def student_detail(request, id):
    ...
    student_course_count = student.courses_count()
    return render(request, 
                  "student/student_detail.html", 
                  {"student": student, "student_course_count": student_course_count})
...
def add_student(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Check for duplicate student (case-insensitive)
            duplicate = Student.objects.filter(
                fname__iexact=new_student.fname,
                lname__iexact=new_student.lname
            ).exists()

            if duplicate:
                message = f"{new_student.fname} {new_student.lname} is already in the system."
                context = {"message": message}
                return render(request, 'student/duplicate_student.html', context)
            
            new_student.save()
            # Student saved, redirect to detail page
...
def deactivate_student(request, id):
    if request.method == "POST":
        ...
        return HttpResponseRedirect(reverse("inactive_students"))
...
def activate_student(request, id):
    if request.method == "POST":
        ...
        messages.success(request, f"{student.fname} {student.lname} has been activated.")
